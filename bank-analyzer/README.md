...
7. Populating `patrimonio_davivienda`, `patrimonio_neto`, and `savings_bank_name` / `credit_bank_name` for dynamic labels.
...
The FastAPI backend exposes the following REST endpoints (base URL: `http://localhost:{port}`):

### Statements
| Method | Path | Description |
|---|---|---|
| `GET` | `/statements/` | List all uploaded statement months |
| `POST` | `/statements/upload` | Upload a PDF (`multipart/form-data`) |
| `DELETE` | `/statements/{id}` | Delete a statement and all its movements |
| `GET` | `/statements/calendar-months` | List distinct `YYYY-MM` calendar months |

### Movements
| Method | Path | Description |
|---|---|---|
| `GET` | `/movements/` | List movements (filter by `month_id`, `calendar_month`, `category_id`, `type`, `search`) |
| `PATCH` | `/movements/{id}` | Update movement category, type, or `applies` flag |

### Summary
| Method | Path | Description |
|---|---|---|
| `GET` | `/summary/{year}/{month}` | Monthly consolidated summary |
| `GET` | `/summary/trends` | Spending trends across all statements |
...
- Overall spending trend card.
- Rising categories count.
- Recurring charges count.
- Expenses-per-statement bar chart.
- Savings balance evolution chart.
...
- Parsers for **Bancolombia, BBVA, and Nequi** are present but have seen limited real-world testing. Expect potential mismatches.
...
| `npm run dev` script uses Windows path separator — fails on macOS/Linux as-is | Workaround: adjust script manually |
