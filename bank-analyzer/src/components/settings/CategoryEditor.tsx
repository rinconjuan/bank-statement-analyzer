...
function CategoryRow({ cat, allCategories, onUpdate, onDelete }: { cat: Category; allCategories: Category[]; onUpdate: (id: number, data: Partial<Omit<Category, 'id'>>) => Promise<void>; onDelete: (id: number, replacementCategoryId?: number) => Promise<void> }) {
  const { t } = useLanguage()
  ...
  const handleDelete = async () => {
    const alternatives = allCategories.filter((c) => c.id !== cat.id)
    const confirmDelete = window.confirm(t('catEditor.confirmDelete', { name: cat.name }))
    if (!confirmDelete) return

    let replacementCategoryId: number | undefined
    if (alternatives.length > 0) {
      const wantsReassign = window.confirm(t('catEditor.confirmReassign'))
      if (wantsReassign) {
        const options = alternatives.map((c) => `${c.id}: ${c.icon} ${c.name}`).join('\n')
        const input = window.prompt(
          `${t('catEditor.reassignPrompt')}\n\n${options}`,
          String(alternatives[0].id)
        )
        if (input === null) return
        const parsed = Number(input)
        const valid = Number.isInteger(parsed) && alternatives.some((c) => c.id === parsed)
        if (!valid) {
          window.alert(t('catEditor.invalidId'))
          return
        }
        replacementCategoryId = parsed
      }
    }

    await onDelete(cat.id, replacementCategoryId)
  }
      ...
      {!editing ? (
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleDelete} className="text-xs px-3 py-1.5 rounded-lg" style={{ background: 'rgba(239,68,68,0.1)', color: 'var(--accent-red)', border: '1px solid rgba(239,68,68,0.2)' }}>
              {t('catEditor.deleteBtn')}
            </button>
          </div>
        </div>
      ) : (
        <div className="space-y-3">
          <div className="flex gap-3">
            <div className="flex flex-col gap-1">
              <label className="text-xs" style={{ color: 'var(--text-muted)' }}>{t('catEditor.labelIcon')}</label>
            ...
            <div className="flex-1 flex flex-col gap-3">
              <div>
              <label className="text-xs block mb-1" style={{ color: 'var(--text-muted)' }}>{t('catEditor.labelName')}</label>
              ...
              <div>
              <label className="text-xs block mb-1" style={{ color: 'var(--text-muted)' }}>{t('catEditor.labelColor')}</label>
          ...
          <div>
            <label className="text-xs block mb-1" style={{ color: 'var(--text-muted)' }}>{t('catEditor.labelKeywords')}</label>
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder={t('catEditor.kwPlaceholder')}
          ...
          <div className="flex gap-2 justify-end">
            <button onClick={() => setEditing(false)} className="text-xs px-3 py-1.5 rounded-lg" style={{ background: 'var(--bg-secondary)', color: 'var(--text-secondary)' }}>{t('catEditor.cancelBtn')}</button>
            <button onClick={handleSave} disabled={saving} className="text-xs px-3 py-1.5 rounded-lg" style={{ background: 'var(--accent-primary)', color: '#fff' }}>{saving ? t('catEditor.saving') : t('catEditor.saveBtn')}</button>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-semibold" style={{ color: 'var(--text-primary)' }}>{t('catEditor.title')}</h2>
        <button
      ...
      {showNew && (
        <div className="rounded-xl p-4 mb-4" style={{ background: 'var(--bg-secondary)', border: '1px solid var(--accent-primary)', borderStyle: 'dashed' }}>
          <h3 className="text-sm font-medium mb-3" style={{ color: 'var(--text-primary)' }}>{t('catEditor.newTitle')}</h3>
          <div className="flex gap-3 mb-3">
            ...
            <div className="flex-1">
              <div className="mb-2">
                <label className="text-xs block mb-1" style={{ color: 'var(--text-muted)' }}>{t('catEditor.labelName')}</label>
                <input value={newName} onChange={(e) => setNewName(e.target.value)} className="w-full text-sm rounded-lg px-3 py-1.5" style={{ background: 'var(--bg-tertiary)', color: 'var(--text-primary)', border: '1px solid var(--border)', outline: 'none' }} placeholder={t('catEditor.namePlaceholder')} />
              </div>
              <div>
                <label className="text-xs block mb-1" style={{ color: 'var(--text-muted)' }}>{t('catEditor.labelColor')}</label>
                <div className="flex gap-1.5 flex-wrap">
          ...
          <div className="mb-3">
            ...
            <div className="flex gap-2">
              <input value={newKeyword} onChange={(e) => setNewKeyword(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addNewKeyword() } }} placeholder={t('catEditor.kwPlaceholder')} className="flex-1 text-xs rounded-lg px-3 py-1.5" style={{ background: 'var(--bg-tertiary)', color: 'var(--text-primary)', border: '1px solid var(--border)', outline: 'none' }} />
          ...
          <div className="flex gap-2 justify-end">
            <button onClick={() => setShowNew(false)} className="text-xs px-3 py-1.5 rounded-lg" style={{ background: 'var(--bg-tertiary)', color: 'var(--text-secondary)' }}>{t('catEditor.cancelBtn')}</button>
            <button onClick={handleCreate} disabled={creating || !newName.trim()} className="text-xs px-3 py-1.5 rounded-lg disabled:opacity-50" style={{ background: 'var(--accent-primary)', color: '#fff' }}>{creating ? t('catEditor.creating') : t('catEditor.createBtn')}</button>
          </div>
        </div>
      )}
      ...
      <div>
        {categories.map((cat) => (
          <CategoryRow key={cat.id} cat={cat} allCategories={categories} onUpdate={onUpdate} onDelete={onDelete} />
        ))}
      </div>
    </div>
  )
